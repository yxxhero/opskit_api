            ...
            if user_ins and user_ins.user_role.code == 1:
                if args.username:
                    if is_auditing is None:
                        user_total = (
                            User.query.filter(User.user_name.contains(args.username))
                        ...
                        user_total = (
                            User.query.filter(
                                User.user_name.contains(args.username),
                                User.is_auditing == is_auditing,
                            )
                            .order_by(User.create_time.desc())
                            .count()
                        )
                    ...
                    if is_auditing is None:
                        user_total = User.query.order_by(
                            User.create_time.desc()
                        ).count()
                        user_ins_list = (
                            User.query.order_by(User.create_time.desc())
                            .limit(args.pagesize)
                            .offset(args.pagesize * (args.page - 1))
                            .all()
                        )
                        ...
                        user_total = (
                            User.query.filter(User.is_auditing == is_auditing)
                            .order_by(User.create_time.desc())
                            .count()
                        )
                        user_ins_list = (
    ...
    def put(self):
        ...
        try:
            ...
            if admin_user and admin_user.user_role.code == 1:
                user_ins = User.query.filter_by(user_name=args.username).first()
                if user_ins:
