...
class AdminNote(Resource):
        ...
        try:
            ...
            if user_ins and user_ins.user_role.code == 1:
                if args.keyword and args.is_public == None:
                    note_total = (
                        Note.query.filter(Note.content.contains(args.keyword))
                    ...
                    note_total = (
                        Note.query.filter(Note.is_public == is_public)
                        .order_by(Note.update_time.desc())
                        .count()
                    )
                    note_ins_list = (
                    ...
                    note_ins_list = (
                        Note.query.order_by(Note.update_time.desc())
                        .limit(args.pagesize)
                        .offset(args.pagesize * (args.page - 1))
                        .all()
                    )
    ...
    def put(self):
        ...
        self.parser.add_argument("is_public", type=bool, required=True, location="json")
        args = self.parser.parse_args()
