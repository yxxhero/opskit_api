...
class Comments(Resource):
    ...
    method_decorators = {"post": [common_decorator]}
    ...
    def get(self):
        ...
        try:
            ...
            comment_list = (
                Comment.query.filter(Comment.state == 1, Comment.note == note)
                .order_by(Comment.create_time.desc())
                .limit(args.page_size)
                .offset(args.page_size * (args.page - 1))
                .all()
            )
            comment_total = Comment.query.filter(
                Comment.state == 1, Comment.note == note
            ).count()
            comment_infos = [
                {
                    "id": item.id,
