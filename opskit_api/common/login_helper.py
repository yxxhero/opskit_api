...
def jwt_encode_token(username):
    ...
    payload = {
        ...
        + datetime.timedelta(
            days=0, seconds=current_app.config.get("TOKEN_DEADLINE", 60)
        ),
        "iat": datetime.datetime.utcnow(),
        "iss": "ken",
        "data": {"username": username, "login_time": login_time},
    }
    return jwt.encode(
...
def jwt_decode_token(jwttoken):
    return jwt.decode(jwttoken, current_app.config["SECRET_KEY"])
    ...
    def wrapper(*args, **kwargs):
        if "Authorization" not in request.headers:
            ...
            try:
                ...
                if redis_store.get(username).decode("utf-8") != request.headers.get(
                    "Authorization"
                ):
